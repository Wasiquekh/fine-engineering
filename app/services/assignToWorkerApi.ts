...
export async function getAssignments(params: {
  ...
  const res = await api.get(`/fineengg_erp/assign-to-worker?${qs.toString()}`);
  return res.data;
}
...
export async function postAssignVendor(id: string, vendor_name: string) {
  ...
  const res = await api.post(
    `/fineengg_erp/assign-to-worker/${id}/assign-vendor`,
    { vendor_name },
...
export async function postQcOutgoing(id: string, payload: any) {
  ...
  const res = await api.post(
    `/fineengg_erp/assign-to-worker/${id}/qc-outgoing`,
    payload,
    { headers: { "Content-Type": "application/json" } } as any
  );
  return res.data;
