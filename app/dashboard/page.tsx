...
import { 
  FiRefreshCw, FiUsers, FiCpu, FiBriefcase, 
...
export default function MainDashboardPage() {
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      // ✅ FIXED: Use correct API endpoints with full path
      const [jobsRes, assignmentsRes, categoriesRes, pendingRes, poRes, activitiesRes] = await Promise.all([
        ...
        axiosProvider.get("/fineengg_erp/system/pending-materials", {
          params: { limit: 1000 },
          headers: undefined
        }),
        axiosProvider.get("/fineengg_erp/system/po-services", {
          params: { limit: 1000 },
          headers: undefined
        }),
  ...
  const assignmentStats = {
    ...
    activeWorkers: new Set(
      assignments
        .filter((a: any) => a.worker_name && a.status !== "completed" && a.status !== "rejected")
        .map((a: any) => a.worker_name)
    ).size,
    totalWorkers: new Set(assignments.filter((a: any) => a.worker_name).map((a: any) => a.worker_name)).size,
  };
  ...
  const pendingByClient = Array.from(pendingByClientMap.entries()).map(([name, data]) => ({
    client_name: name,
    count: data.count,
    qty: data.qty
  }));
  ...
  const jobTypeChartData = {
    ...
    datasets: [
      {
        label: 'In Progress',
        data: [jobStats.inProcess, jobStats.tsoInProcess, jobStats.kanbanInProcess],
        backgroundColor: '#F59E0B',
        borderRadius: 8,
      },
      {
        label: 'Completed',
        data: [jobStats.completed, jobStats.tsoCompleted, jobStats.kanbanCompleted],
        ...
        <div className="p-4 md:p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
            <div className="bg-white rounded-xl shadow-sm p-3 text-center">
              ...
              <p className="text-xl font-bold text-blue-600">{jobStats.jobService}</p>
            </div>
            <div className="bg-white rounded-xl shadow-sm p-3 text-center">
              <p className="text-xs text-gray-500">TSO Service</p>
              <p className="text-xl font-bold text-purple-600">{jobStats.tso}</p>
            </div>
            <div className="bg-white rounded-xl shadow-sm p-3 text-center">
              <p className="text-xs text-gray-500">Kanban</p>
              <p className="text-xl font-bold text-green-600">{jobStats.kanban}</p>
            </div>
            <div className="bg-white rounded-xl shadow-sm p-3 text-center">
              <p className="text-xs text-gray-500">PO Services</p>
              <p className="text-xl font-bold text-orange-600">{poStats.total}</p>
            </div>
            <div className="bg-white rounded-xl shadow-sm p-3 text-center">
              <p className="text-xs text-gray-500">Categories</p>
              <p className="text-xl font-bold text-teal-600">{categoryStats.total}</p>
            </div>
          </div>
            ...
            <>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <div className="bg-white rounded-xl shadow-sm p-5">
                  ...
                  <Bar data={jobTypeChartData} options={{ responsive: true, scales: { x: { stacked: true }, y: { stacked: true, beginAtZero: true } } }} />
                </div>
                <div className="bg-white rounded-xl shadow-sm p-5">
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">Assignment Status</h3>
                  <div className="flex justify-center">
                    <Doughnut data={assignmentStatusData} options={{ responsive: true, plugins: { legend: { position: "bottom", labels: { font: { size: 10 } } } } }} />
                  </div>
                </div>
              </div>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {categories.slice(0, 10).map((cat: any) => (
                          <tr key={cat.id} className="hover:bg-gray-50">
                            <td className="px-4 py-3 text-sm font-medium text-gray-900">{cat.job_no}</td>
                            <td className="px-4 py-3 text-sm">
                              <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs">{cat.job_category || 'N/A'}</span>
                            </td>
                            <td className="px-4 py-3 text-sm text-gray-600">{cat.description || '-'}</td>
                            <td className="px-4 py-3 text-sm font-semibold">{cat.qty}</td>
                            <td className="px-4 py-3 text-sm text-gray-600">{cat.client_name || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {urgentJobs.slice(0, 10).map((job: any) => (
                          <tr key={job.id} className="hover:bg-gray-50">
                            <td className="px-4 py-3 text-sm text-gray-900">{job.job_no || job.tso_no}</td>
                            <td className="px-4 py-3 text-sm">
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                job.job_type === "JOB_SERVICE" ? "bg-blue-100 text-blue-700" :
                                job.job_type === "TSO_SERVICE" ? "bg-purple-100 text-purple-700" : "bg-green-100 text-green-700"
                              }`}>
                                {job.job_type === "JOB_SERVICE" ? "Job Service" : job.job_type === "TSO_SERVICE" ? "TSO" : "Kanban"}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-sm text-gray-600">{job.client_name || '-'}</td>
                            <td className="px-4 py-3 text-sm text-gray-600">{job.qty}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">In Review</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Not OK</th>
                    ...
                    {workersList.length > 0 ? (
                      workersList.map((worker: any) => (
                        <tr key={worker.worker_name} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-green-600 font-semibold">{worker.completed_count}</td>
                          <td className="px-4 py-3 text-sm text-yellow-600">{worker.in_progress_count}</td>
                          <td className="px-4 py-3 text-sm text-blue-600">{worker.in_review_count}</td>
                          <td className="px-4 py-3 text-sm text-red-600">{worker.not_ok_count}</td>
                          <td className="px-4 py-3 text-sm">
                            <div className="flex items-center gap-2">
                              <div className="w-20 bg-gray-200 rounded-full h-2">
                                <div className="bg-green-500 h-2 rounded-full" style={{ width: `${worker.completion_rate}%` }} />
                              </div>
                              <span className="text-xs font-semibold">{worker.completion_rate}%</span>
                            </div>
                          </td>
                        </tr>
                      ))
                    ) : (
                      <tr><td colSpan={8} className="text-center py-8 text-gray-500">No worker data available</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
                        ...
                        <tr key={machine.machine_category} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs font-semibold">{machine.utilization_rate}%</span>
                            </div>
                          </td>
                        </tr>
                      ))
                    ) : (
                      <tr><td colSpan={6} className="text-center py-8 text-gray-500">No machine data available</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-5">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-yellow-50 rounded-lg p-4 text-center">
                  <p className="text-2xl font-bold text-yellow-600">{pendingStats.pending}</p>
                  <p className="text-xs text-gray-500">Pending Items</p>
                </div>
                <div className="bg-green-50 rounded-lg p-4 text-center">
                  <p className="text-2xl font-bold text-green-600">{pendingStats.completed}</p>
                  ...
                  {pendingByClient.map((client: any) => (
                    <div key={client.client_name} className="mb-3">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div className="bg-orange-500 h-2 rounded-full" style={{ width: `${(client.qty / (pendingStats.totalQuantity || 1)) * 100}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-blue-50 rounded-lg p-3 text-center">
                  <p className="text-2xl font-bold text-blue-600">{poStats.total}</p>
                ...
                <div className="bg-red-50 rounded-lg p-3 text-center">
                  <p className="text-2xl font-bold text-red-600">{poStats.rejected}</p>
                  <p className="text-xs text-gray-500">Rejected</p>
                </div>
              </div>
              <div className="bg-blue-50 rounded-lg p-4 text-center">
                <p className="text-sm text-gray-600">Total Amount</p>
                <p className="text-2xl font-bold text-blue-600">₹{poStats.totalAmount.toLocaleString()}</p>
              </div>
            </div>
          )}
        </div>
