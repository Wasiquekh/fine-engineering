...
export default function MaterialsMasterPage() {
  ...
  const handleSearchMaterial = async (val: string, field: string) => {
    ...
    try {
      ...
      const res = await axiosProvider.get(`/fineengg_erp/system/inventory3-materials?search=${val}`);
  ...
  const fetchData = async () => {
    try {
      const res = await axiosProvider.get("/fineengg_erp/system/inventory3-pr");
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axiosProvider.delete(`/fineengg_erp/system/inventory3-pr/${id}`);
  ...
  const handleSubmit = async (values: any, actions: any) => {
    try {
      ...
      await axiosProvider.post("/fineengg_erp/system/inventory3-pr", payload);
        ...
        <div className="w-full md:w-[83%] bg-[#F5F7FA] min-h-screen p-4 relative">
          ...
          <div className="rounded-3xl shadow bg-white px-4 py-6 relative">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-800">Inventory 3 PR</h2>
              <button
                onClick={openFlyout}
