...
export default function Home() {
  ...
  const handleFilterChange = (newFilter: string) => {
    ...
    router.push(`/section_inventory/inventory_material_approve?${currentParams.toString()}`);
  };
  ...
  const handleGroupClick = (group: JobGroup) => {
    ...
    if (job_type === "TSO_SERVICE") {
      router.push(`/section_inventory/tso_approve/${encodeURIComponent(groupId)}${queryString}`);
    } else if (job_type === "KANBAN") {
      router.push(`/section_inventory/kanban_approve/${encodeURIComponent(groupId)}${queryString}`);
    } else {
      // Default behavior for JOB_SERVICE or other types
      router.push(
        `/section_inventory/inventory_material_approve/${encodeURIComponent(groupId)}${queryString}`
      );
    }
  };
